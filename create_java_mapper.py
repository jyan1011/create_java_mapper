...
def getAllColumns(cursor, tableName):
    # 根据数据库以及表名获取所有字段和备注
    sql = "select COLUMN_NAME,COLUMN_TYPE,column_comment from INFORMATION_SCHEMA.Columns where table_name = '%s' and table_schema = '%s' "
...
# 创建实体类内容
def createModel(packageName, tableName, columns):
...
if __name__ == '__main__':
    ...
    for t in getAllTables(cursor):
        ...
        createJavaFile(config.path + config.model.replace('.', '/'),
                       ...
                       createModel(config.model, tableName, column))
        # 创建mapper对象
